...
console.log('Checking sanity of settings....');
...
ops.read_folder = function (callback) {
    dir.files('/data' + dataset.datasets, function (err, files) {
        ...
        console.log('Found', _.size(files), 'datasets in folder.')
        callback();
    });
};
...
ops.upload_mvel = function (callback) {
    ...
    token(function (err, access_token) {
        ...
        .end(function (err, res) {
            ...
            tmp.status = res.body;
            console.log('Uploaded MVEL dataset.');
            callback(err);
        });
    });
...
ops.upload_datasets = function (callback) {
    ...
    async.eachSeries(files, function (file, done) {
        ...
        token(function (err, access_token) {
            ...
            .end(function (err, response) {
                if (err) {
                    console.log('There was an error uploading a dataset:', err);
                    return done();
                }
                ...
                console.log('Uploaded dataset', n, 'of', p);
                n++;
                done();
            });
        });
...
ops.add_datasets_to_layer = function (callback) {
    ...
    token(function (err, access_token) {
        ...
        .end(function (err, response) {
            ...
            console.log('Added datasets to layer.');
            callback();
        });
    });
...
async.series(ops, function (err, results) {
    ...
    if (err) {
        ...
        console.log('Your data was uploaded successfully.')
        console.log('Please open the project', tmp.project.name);
    }
});
